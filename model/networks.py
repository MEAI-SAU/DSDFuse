...
def weights_init_kaiming(m, scale=1):
    classname = m.__class__.__name__
    if classname.find('Conv2d') != -1 and hasattr(m, 'weight') and m.weight is not None:
...
def define_Network(opt,local_rank):
    from model.head.fusion_pipeline import FusionPipeline
    ...
    Fusion_model = FusionPipeline(
        ...
        efficiency=model_opt['Fusion'].get('efficiency'),
        ablation=model_opt.get('ablation'),
    )

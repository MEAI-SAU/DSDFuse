...
EPS = 1e-12
PAPER_METRICS = ("MI", "VIF", "QABF", "SSIM", "FMI_pixel", "SCD", "NCIE", "AG")
...
def calculate_ssim_metric(x, y, f):
    x = ensure_gray_u8(x)
    y = ensure_gray_u8(y)
    f = ensure_gray_u8(f)
    ...
    return float((ssim_x + ssim_y) / 2.0)
...
def write_metric_reports(output_folder, records, metrics, efficiency_info):
    ...
    per_image_fields = ["image", "MI", "VIF", "QABF", "SSIM", "FMI_pixel", "SCD", "NCIE", "AG"]
...
if __name__ == "__main__":
    ...
    parser.add_argument("-c", "--config", type=str, default="config/test_spect_mri.json")
    ...
    with torch.no_grad():
        for img_name in tqdm(image_names, desc="Processing images"):
            ...
            spect_pil = Image.open(os.path.join(test_folder, "SPECT", img_name)).convert("RGB")
            mri_pil = Image.open(os.path.join(test_folder, "MRI", img_name)).convert("L")

            spect_rgb = np.array(spect_pil, dtype=np.uint8)
            mri_gray = np.array(mri_pil, dtype=np.uint8)
            spect_gray = cv2.cvtColor(spect_rgb, cv2.COLOR_RGB2GRAY)

            spect_tensor = (ToTensor()(spect_pil) * (min_max[1] - min_max[0]) + min_max[0]).unsqueeze(0).to(device)
            mri_tensor = (ToTensor()(mri_pil) * (min_max[1] - min_max[0]) + min_max[0]).unsqueeze(0).to(device)

            spect_yuv = RGB2YCrCb(spect_tensor)
            spect_y = spect_yuv[:, 0:1, :, :]
            mri_y = mri_tensor[:, 0:1, :, :]
            input_tensor = torch.cat([mri_y, spect_y], dim=1)

            fused_y_tensor = diffusion.Fusion_net.test_Fusion(input_tensor, device)
            fused_rgb_tensor = YCrCb2RGB(torch.cat((fused_y_tensor, spect_yuv[:, 1:2, :, :], spect_yuv[:, 2:3, :, :]), dim=1))
            ...
            spect_eval = cv2.resize(spect_gray, (target_w, target_h), interpolation=cv2.INTER_LINEAR)
            mri_eval = cv2.resize(mri_gray, (target_w, target_h), interpolation=cv2.INTER_LINEAR)

            record = OrderedDict(
                [
                    ("image", img_name),
                    ("MI", calculate_mi(spect_eval, mri_eval, fused_gray)),
                    ("VIF", calculate_vif(spect_eval, mri_eval, fused_gray)),
                    ("QABF", calculate_qabf(spect_eval, mri_eval, fused_gray)),
                    ("SSIM", calculate_ssim_metric(spect_eval, mri_eval, fused_gray)),
                    ("FMI_pixel", calculate_fmi_pixel(spect_eval, mri_eval, fused_gray)),
                    ("SCD", calculate_scd(spect_eval, mri_eval, fused_gray)),
                    ("NCIE", calculate_ncie(spect_eval, mri_eval, fused_gray)),
                    ("AG", calculate_ag(fused_gray)),
                ]
            )
            ...
            print(f"\nImage: {img_name}")
            print(f"  MI      : {record['MI']:.4f}")
            print(f"  VIF     : {record['VIF']:.4f}")
            print(f"  QABF    : {record['QABF']:.4f}")
            print(f"  SSIM    : {record['SSIM']:.4f}")
            print(f"  FMI_px  : {record['FMI_pixel']:.4f}")
            print(f"  SCD     : {record['SCD']:.4f}")
            print(f"  NCIE    : {record['NCIE']:.4f}")
            print(f"  AG      : {record['AG']:.4f}")
    ...
    print("\n" + "=" * 72)
    print("SPECT-MRI fusion metrics (mean +- std)")
    print("-" * 72)

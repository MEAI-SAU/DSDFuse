...
# Usage: bash scripts/run_ablation_spect.sh

echo "Starting SPECT-MRI ablation experiments..."
echo "=========================================="
...
CONFIG_PREFIX="config/train_spect_mri_ablation_"
...
for mode in "${ABLATION_MODES[@]}"; do
    ...
    CHECKPOINT_DIR="experiments/ablation/SPECT_MRI/${mode}/checkpoint"
    ...
    if [ -n "$BEST_MODEL" ]; then
        ...
        python test-med-PET.py --model_path "$BEST_MODEL" --output_dir "experiments/ablation/SPECT_MRI/${mode}/fused_images" --save_metrics "experiments/ablation/SPECT_MRI/${mode}/metrics.csv" --dataset "SPECT-MRI"
    else
...
echo ""
echo "========================================"
echo "SPECT-MRI ablation experiments completed!"
echo "========================================"